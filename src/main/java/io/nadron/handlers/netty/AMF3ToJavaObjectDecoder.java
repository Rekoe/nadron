...
public class AMF3ToJavaObjectDecoder extends ByteToMessageDecoder implements Transform<ByteBuf, Object> {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in,
        ...
        ByteArrayInputStream bis = new ByteArrayInputStream(in.readBytes(in.readableBytes()).array());
        Object value = null;
        try {
            value = deSerializeObjectFromStream(bis);
        } catch (Exception e) {
            LOG.trace("invalid object to deserialize");
            throw e;
        }
        out.add(value);
    }
